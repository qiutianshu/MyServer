...
typedef struct {
  ...
  unsigned int gather_statistics;
...
METHODDEF(unsigned int) encode_mcu_DC_first JPP((j_compress_ptr cinfo,
					    JBLOCKROW *MCU_data));
METHODDEF(unsigned int) encode_mcu_AC_first JPP((j_compress_ptr cinfo,
					    JBLOCKROW *MCU_data));
METHODDEF(unsigned int) encode_mcu_DC_refine JPP((j_compress_ptr cinfo,
					     JBLOCKROW *MCU_data));
METHODDEF(unsigned int) encode_mcu_AC_refine JPP((j_compress_ptr cinfo,
					     JBLOCKROW *MCU_data));
...
METHODDEF(void)
start_pass_phuff (j_compress_ptr cinfo, unsigned int gather_statistics)
{  
  phuff_entropy_ptr entropy = (phuff_entropy_ptr) cinfo->entropy;
  unsigned int is_DC_band;
  int ci, tbl;
...
finish_pass_gather_phuff (j_compress_ptr cinfo)
{
  ...
  jpeg_component_info * compptr;
  JHUFF_TBL **htblptr;
  unsigned int did[NUM_HUFF_TBLS];
