...
typedef struct {
  ...
  unsigned int suspended;		/* remember if we suspended output */
			  ...
			  JDIMENSION in_rows_avail)
{
  ...
  unsigned int writing = (main->pass_mode != JBUF_CRANK_DEST);
...
GLOBAL(void)
jinit_c_main_controller (j_compress_ptr cinfo, unsigned int need_full_buffer)
{
  my_main_ptr main;
  int ci;
  jpeg_component_info *compptr;
