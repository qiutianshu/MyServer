...
#endif
#ifdef BLOCK_SMOOTHING_SUPPORTED
LOCAL(unsigned int) smoothing_ok JPP((j_decompress_ptr cinfo));
...
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  unsigned int smoothing_useful = FALSE;
  int ci, coefi;
  jpeg_component_info *compptr;
  JQUANT_TBL * qtable;
  int * coef_bits;
...
GLOBAL(void)
jinit_d_coef_controller (j_decompress_ptr cinfo, unsigned int need_full_buffer)
{
  my_coef_ptr coef;
