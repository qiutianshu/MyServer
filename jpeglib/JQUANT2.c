...
typedef struct {
  ...
  unsigned int needs_zeroed;		/* TRUE if next pass must zero histogram */

  /* Variables for Floyd-Steinberg dithering */
  FSERRPTR fserrors;		/* accumulated errors */
  unsigned int on_odd_row;		/* flag to remember which row we are on */
...
METHODDEF(void)
start_pass_2_quant (j_decompress_ptr cinfo, unsigned int is_pre_scan)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  hist3d histogram = cquantize->histogram;
  int i;
