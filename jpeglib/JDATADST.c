...
empty_output_buffer (j_compress_ptr cinfo)
{
  ...
  {
	if(cinfo->l_size >= cinfo->l_point + OUTPUT_BUF_SIZE)
	{
	 for (l  = 0; l < OUTPUT_BUF_SIZE; l++)
		 cinfo->buffer[cinfo->l_point + l] = dest->buffer[l];
	}
	 cinfo->l_point += OUTPUT_BUF_SIZE;
  }
...
term_destination (j_compress_ptr cinfo)
{
  ...
  if (datacount > 0) {
//DEL    if (JFWRITE(dest->outfile, dest->buffer, datacount) != datacount)
//DEL      ERREXIT(cinfo, JERR_FILE_WRITE);
  if (!cinfo->i_stream)
  {
  ...
  {
	if(cinfo->l_size >= cinfo->l_point + datacount)
	{
	 for (l  = 0; l < datacount; l++)
		 cinfo->buffer[cinfo->l_point + l] = dest->buffer[l];
	}
	 cinfo->l_point += datacount;
  }
