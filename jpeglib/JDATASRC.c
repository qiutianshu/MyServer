...
typedef struct {
  ...
  unsigned int start_of_file;	/* have we gotten any data yet? */
} my_source_mgr;
...
fill_input_buffer (j_decompress_ptr cinfo)
{
  ...
  size_t nbytes;
  unsigned long l;
//DEL  nbytes = JFREAD(src->infile, src->buffer, INPUT_BUF_SIZE);
  if (!cinfo->i_stream)
  {
  ...
  {
	 if (cinfo->l_size >= cinfo->l_point + INPUT_BUF_SIZE)
	   nbytes = INPUT_BUF_SIZE;
	 else
	   nbytes = cinfo->l_size - cinfo->l_point;
//	 lstrcpyn(src->buffer,cinfo->buffer+cinfo->l_point, 
//		 nbytes);
	 for (l  = 0; l < nbytes; l++)
		 src->buffer[l] = cinfo->buffer[cinfo->l_point + l];
	 cinfo->l_point += nbytes;
  }
  if (nbytes <= 0) {
